...
class TestNaturalSortKey:
    ...
    def test_mixed_formats(self):
        ...
        files = [
            'image_1.ply', 'image_10.ply', 'image_2.ply',
            'frame_001.ply', 'frame_100.ply', 'frame_002.ply'
        ]
        sorted_files = sorted(files, key=natural_sort_key)

        # Should group by prefix, then sort numerically
        assert sorted_files == [
            'frame_001.ply', 'frame_002.ply', 'frame_100.ply',
...
class TestNaturalSortIntegration:
    ...
    def test_ply_model_uses_natural_sort(self, sample_ply_files):
        ...
        for i in range(len(model.ply_files) - 1):
            ...
            assert current_num < next_num, f"Files not in natural order: {current_file} -> {next_file}"
