...
This module is adapted from the upstream nerfview project so that we can evolve
it locally without pulling an external dependency.
"""
    ...
    def run(self):
        while self.running:
            ...
            try:
                ...
                traceback.print_exc()
                os._exit(1)
            # Use configured JPEG quality from viewer settings
            jpeg_quality = (
                self.viewer.jpeg_quality_static
                if task.action in ["static", "update"]
                else self.viewer.jpeg_quality_move
            )
