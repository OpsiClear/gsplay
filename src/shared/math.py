...
This module contains math helper functions including KNN, random seeding,
color transformations, rotation conversions, and other mathematical operations.
"""
...
def knn(x: Tensor, K: int = 4) -> Tensor:
    ...
    # Safety check: if we have fewer points than K, use all available points
    n_samples = x_np.shape[0]
    if n_samples < K:
        logger.warning(f"Only {n_samples} samples available, but {K} neighbors requested. Using {n_samples} neighbors instead.")
        K = n_samples

    # If we only have 1 point, return a tensor with a single distance of 0
    if n_samples == 1:
        return torch.zeros((1, 1), device=x.device, dtype=x.dtype)

    tree = cKDTree(x_np)
    distances, _ = tree.query(x_np, k=K)
...
def colormap(img: torch.Tensor, cmap: str = "jet") -> torch.Tensor:
    """Apply a colormap to an image with a colorbar.

    Args:
        img: Image tensor with shape (W, H) or (W, H, C)
        cmap: Matplotlib colormap name (default: "jet")

    Returns:
        RGB image tensor with colorbar applied, shape (3, H_new, W_new)
    """
...
def apply_depth_colormap(
    ...
    Args:
        depth: (..., 1) float32 depth.
        acc: (..., 1) optional accumulation mask.
...
def find_k_neighbors(input: torch.Tensor, k: int) -> tuple[torch.Tensor, torch.Tensor]:
    """
    ...
    Args:
        input: point cloud data with shape (n, 3)
